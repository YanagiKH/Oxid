...
> [!CAUTION]
> 現時点では、このリポジトリではRustが依然として「fallback boundary」として扱われているため、依存関係として [Rust](https://github.com/rust-lang/rust) をインストールする必要があります。
...
- `docs/` には、ワークフロー、コンパイラ、構文、診断、相互運用性、セキュリティ、およびネットワークアプリケーションに関するドキュメントが含まれています
...
- 最新のネットワークサービス、Web API、Discordボット、その他のイベント駆動型アプリケーションに向けた明確な道筋
